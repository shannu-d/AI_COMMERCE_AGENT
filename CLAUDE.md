...
(approval) and M9 (Policy Engine) are complete. M10–M15 are not started.**
...
with a database: **1115 tests, all passing, none skipped**; 825 of those need no database.

This machine has neither Docker nor an installed PostgreSQL. The documented way around that — used
to verify M1, M3 and M5 through M9 — is a throwaway PostgreSQL 16.4 unpacked from the official Windows binary
...
worked example, `tests/ranking/test_ranker.py`) an ordinary unit test, and it is why 825 of the 1115
