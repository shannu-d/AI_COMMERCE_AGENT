...
def test_no_commerce_service_or_policy_module_has_been_created_early() -> None:
    ...
    whether money may move without moving any. The Order Service is M10 and the
    Razorpay client M11, and the specification is emphatic (D§39, A§58, F§37)
    that the money path must not be coded before its decisions exist. Tables are
    inert; a service that writes to them is not.
    """
    packages = [
        name
        for name in ("app/payments", "app/services/order_service.py")
